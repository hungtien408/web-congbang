        ...
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["nid"]))
            {
                ...
                if (dv.Count > 0)
                {
                    ...
                    Repeater1.DataBind();
                    strTitle = Server.HtmlDecode(row["ProjectTitleCam"].ToString());
                    strMetaTitle = Server.HtmlDecode(row["MetaTittleCam"].ToString());
            ...
            else  if (!string.IsNullOrEmpty(Request.QueryString["nd"]))
            {
